...
let _lastRenderView = null, _lastRenderSub = "";   // pour préserver le scroll quand on re-rend la même vue
...
/* Conteneur scrollable actif : panneau du pager sur mobile, sinon .content. */
function activeScroller() {
  return (isMobile() && Pager.has()) ? Pager.activePane() : $(".content");
}

/* Remonte en haut le conteneur scrollable actif, en douceur. */
...
function renderApp() {
  ...
  // Préserver le scroll quand on re-rend la MÊME vue (mise à jour de données, pas une navigation).
  const curSub = _sub[_view] || "";
  const sameView = _lastRenderView === _view && _lastRenderSub === curSub && app.childElementCount > 0;
  const prevScroll = sameView ? (activeScroller()?.scrollTop || 0) : 0;
  app.innerHTML = "";
  ...
  // Restaurer la position de défilement si c'est la même vue (sinon, on reste en haut).
  if (sameView && prevScroll) {
    const sc = activeScroller();
    if (sc) sc.scrollTop = prevScroll;
  }
  _lastRenderView = _view;
  _lastRenderSub = curSub;
