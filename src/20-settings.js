...
function viewSettings(root) {
  ...
  const contentCard = el("div", { class: "card card-pad" },
    el("h3", { class: "mb12" }, "🗂️ Personnaliser mon budget"),
    el("p", { class: "small muted mb12" }, "Adaptez les catégories de revenus et de dépenses à votre situation : ajoutez, renommez ou retirez ce qui ne vous concerne pas."),
    el("div", { class: "flex", style: "flex-wrap:wrap; gap:8px" },
      el("button", { class: "btn btn-p", html: ico("list", 15) + "<span>Gérer les catégories</span>", onclick: () => b && openCategoryManager(b) }),
      el("button", { class: "btn", html: ico("layers", 15) + "<span>Mes budgets</span>", onclick: () => go("budgets") }),
      el("button", { class: "btn", html: ico("info", 15) + "<span>Aide & tutoriels</span>", onclick: () => go("help") }))
  );
